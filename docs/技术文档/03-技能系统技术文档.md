...
**技能（Skills）** 是 DeerFlow 的模块化扩展机制，将特定领域的知识、工具和流程打包为可复用单元。
...
**路径转换源码**（`backend/packages/harness/deerflow/skills/types.py#L24-L38`）：
...
**1. Skill 数据类**（`backend/packages/harness/deerflow/skills/types.py#L5-L16`）
...
**2. 技能加载器**（`backend/packages/harness/deerflow/skills/loader.py#L25-L101`）
...
**3. 技能解析器**（`backend/packages/harness/deerflow/skills/parser.py`）
...
**技能提示词组装源码**（`backend/packages/harness/deerflow/agents/lead_agent/prompt.py#L383-L424`）
...
**安装器源码**（`backend/packages/harness/deerflow/skills/installer.py#L117-L136`）
...
**上一篇**：[02-DeerFlow 架构分析](02-DeerFlow%20架构分析.md)  
