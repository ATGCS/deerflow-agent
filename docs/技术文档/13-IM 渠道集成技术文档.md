...
IM 渠道集成让 DeerFlow 能够接入即时通讯平台（如飞书、钉钉、企业微信），用户可以在熟悉的聊天工具中直接与 AI Agent 对话，无需切换应用。
...
class FeishuAdapter:
    ...
    async def _handle_message(self, event: dict) -> None:
        ...
        # 调用 DeerFlow
        response = await self._call_deerflow(thread_id, text)
...
class IMAdapter(ABC):
    ...
    async def _call_deerflow(self, thread_id: str, message: str) -> str:
        """调用 DeerFlow Agent。"""
