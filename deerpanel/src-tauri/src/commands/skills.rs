...
/// 列出所�?Skills 及其状态（deerpanel skills list --json�?#[tauri::command]
pub async fn skills_list() -> Result<Value, String> {
    ...
    match output {
            ...
            match extract_json(&stdout) {
                ...
                None => {
                    ...
                    scan_local_skills(Some(serde_json::json!({
                        "status": "parse-failed",
                        "message": "DeerPanel CLI 可执行，但返回结果未能解析为 JSON，当前展示本地扫描结�?,
                        "cliAvailable": true,
        ...
        Ok(Err(e)) => scan_local_skills(Some(serde_json::json!({
            "status": "exec-failed",
            "message": format!("调用 DeerPanel CLI 失败，当前展示本地扫描结�? {e}"),
            "cliAvailable": false,
        }))),
        Err(_) => scan_local_skills(Some(serde_json::json!({
            "status": "timeout",
            "message": "DeerPanel CLI 调用超时，当前展示本地扫描结�?,
            "cliAvailable": true,
            "timeoutSeconds": 15,
        }))),
    }
...
/// 查看单个 Skill 详情（openclaw skills info <name> --json�?#[tauri::command]
...
/// 检�?Skills 依赖状态（deerpanel skills check --json�?#[tauri::command]
pub async fn skills_check() -> Result<Value, String> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(format!("检查失�? {}", stderr.trim()));
    }
...
/// 安装 Skill 依赖（根�?install spec 执行 brew/npm/go/uv/download�?#[tauri::command]
...
/// 检�?SkillHub CLI 是否已安�?#[tauri::command]
...
/// 安装 SkillHub CLI（从腾讯�?COS 下载�?#[tauri::command]
pub async fn skills_skillhub_setup(cli_only: bool) -> Result<Value, String> {
    ...
    #[cfg(target_os = "windows")]
    {
        // Windows: 通过 npm 全局安装 skillhub（避�?bash/WSL 路径问题�?        let mut cmd = tokio::process::Command::new("cmd");
        cmd.args([
            "/c",
            "npm",
            "install",
...
/// �?SkillHub 安装 Skill（skillhub install <slug>�?#[tauri::command]
pub async fn skills_skillhub_install(slug: String) -> Result<Value, String> {
    ...
    let output = cmd
        .output()
        .await
        .map_err(|e| format!("执行 skillhub 失败: {e}。请先安�?SkillHub CLI"))?;
...
/// �?SkillHub 搜索 Skills（skillhub search <query>�?#[tauri::command]
pub async fn skills_skillhub_search(query: String) -> Result<Value, String> {
    ...
    // skillhub search 实际输出格式�?    // ──────────────── (分隔�?
    // [1]   deerpanel/deerpanel/feishu-doc           🛡�?Pass
    //      AI 85  �?    33  �?248.7k  Feishu document read/write opera...
    // ──────────────── (分隔�?
    // 序号�?slug 在同一行，描述在下一�?    let lines: Vec<&str> = stdout.lines().collect();
    let mut items: Vec<Value> = Vec::new();

    for (i, line) in lines.iter().enumerate() {
        let trimmed = line.trim();
        // 找序号行：以 [数字] 开头，同一行包�?slug（owner/repo/name�?        if !trimmed.starts_with('[') {
            continue;
        }
        ...
        // 提取 ] 后面的内�?        let after_bracket = trimmed[bracket_end + 1..].trim();
        // slug 是第一个空格前的部分，且包�?/
        ...
        if i + 1 < lines.len() {
            ...
            // �? "�?     0  �?212.2k  Feishu document..."
            // 策略：找 �?后面的数字后的文�?            if let Some(star_pos) = next.find('�?) {
                let after_star = &next[star_pos + '�?.len_utf8()..].trim_start();
                // 跳过星标数字（如 "248.7k"�?                let after_num = after_star
...
/// �?ClawHub 搜索 Skills（npx clawhub search <query>）�?原版海外�?#[tauri::command]
...
/// �?ClawHub 安装 Skill（npx clawhub install <slug>）�?原版海外�?#[tauri::command]
...
/// 卸载 Skill（删�?~/.deerpanel/skills/<name>/ 目录�?#[tauri::command]
pub async fn skills_uninstall(name: String) -> Result<Value, String> {
    if name.is_empty() || name.contains("..") || name.contains('/') || name.contains('\\') {
        return Err("无效�?Skill 名称".to_string());
    }
    let skills_dir =
...
pub async fn skills_validate(name: String) -> Result<Value, String> {
    ...
    if !skill_md.exists() {
        issues.push(serde_json::json!({
            ...
            "suggestion": "创建 SKILL.md 文件，包�?skill 的描述和使用说明"
        }));
    } else {
        passed.push("SKILL.md 存在".to_string());

        // 2. 检�?SKILL.md frontmatter 格式
        if let Some(frontmatter) = parse_skill_frontmatter(&skill_md) {
            // 检查必要字�?            let required_fields = ["description", "fullPath"];
            for field in &required_fields {
                    ...
                    .unwrap_or(false)
                {
                    issues.push(serde_json::json!({
                        ...
                        "field": field,
                        "suggestion": format!("�?frontmatter 中添�?{}: <�?", field)
                    }));
                } else {
                    passed.push(format!("frontmatter.{} 字段存在且非�?, field));
                }
            }

            // 检�?fullPath 格式（应该是绝对路径�?~ 开头）
            if let Some(fp) = frontmatter.get("fullPath").and_then(|v| v.as_str()) {
                // Windows 路径以盘符开头（�?C:\），Unix �?/ �?~ �?. 开�?                let is_valid_path = fp.starts_with('/')
                    || fp.starts_with('~')
                    || fp.starts_with('.')
                    || (fp.len() >= 3
                        && fp.as_bytes()[1] == b':'
                ...
                if !is_valid_path {
                    warnings.push(serde_json::json!({
                        ...
                        "message": format!("fullPath 格式可能不正�? {}", fp),
                        "suggestion": "建议使用绝对路径�?~ 开�?
                    }));
                }
            }
        } else {
            issues.push(serde_json::json!({
                ...
                "message": "SKILL.md frontmatter 格式不正�?,
                "suggestion": "确保 frontmatter �?--- 开头和结尾，包含正确的 YAML 格式"
            }));
        }

        // 3. 检�?SKILL.md 内容（非 frontmatter 部分�?        if let Ok(content) = std::fs::read_to_string(&skill_md) {
            // 检查是否有空内�?            let body = content
                .split("---")
                ...
                if let Some(pkg_name) = pkg.get("name").and_then(|v| v.as_str()) {
                    if pkg_name != name {
                        warnings.push(serde_json::json!({
                            "level": "warning",
                            "code": "NAME_MISMATCH",
                            "message": format!("package.json 中的 name '{}' 与目录名 '{}' 不一�?, pkg_name, name),
                            "suggestion": "确保 package.json �?name 字段�?skill 目录名一�?
                        }));
                    } else {
                        passed.push("package.json.name 与目录名一�?.to_string());
                    }
                }

                // 检�?dependencies �?node_modules
                if let Some(deps) = pkg.get("dependencies").and_then(|v| v.as_object()) {
                    let deps_count = deps.len();
                    passed.push(format!("package.json 声明�?{} 个依�?, deps_count));
                    ...
                    if node_modules.exists() {
                        ...
                        if !missing.is_empty() {
                            warnings.push(serde_json::json!({
                                ...
                                "message": format!("缺少 {} �?npm 依赖: {}", missing.len(), missing.join(", ")),
                                "missingDeps": missing,
                                "suggestion": "运行 npm install 安装依赖"
                            }));
                        } else {
                            passed.push("所�?npm 依赖已安�?.to_string());
                        }
                    } else if deps_count > 0 {
                        issues.push(serde_json::json!({
                            "level": "error",
                            "code": "NODE_MODULES_MISSING",
                            "message": "package.json 声明了依赖但 node_modules 不存�?,
                            "suggestion": "运行 npm install 安装依赖"
                        }));
                    }
                }
    ...
    for file in unnecessary_files {
        ...
        if file_path.exists() {
            warnings.push(serde_json::json!({
                ...
                "suggestion": "Skill 文档应放�?SKILL.md 中，删除 README.md"
            }));
        }
    }

    // 汇总结�?    let has_errors = !issues.is_empty();
...
fn custom_skill_roots() -> Vec<(std::path::PathBuf, &'static str)> {
    let mut roots = vec![(super::deerpanel_dir().join("skills"), "DeerPanel 自定�?)];
    if let Some(home) = dirs::home_dir() {
        let claude_skills = home.join(".claude").join("skills");
        if !roots.iter().any(|(dir, _)| dir == &claude_skills) {
            roots.push((claude_skills, "Claude 自定�?));
        }
    }
    roots
}
...
/// CLI 不可用或当前结果不可用时的兜底：扫描本地自定�?Skills 目录（含 ~/.deerpanel/skills �?~/.claude/skills�?fn scan_local_skills(cli_diagnostic: Option<Value>) -> Result<Value, String> {
...
/// 扫描单个 Skill 的详细信�?fn scan_single_skill(skill_path: &std::path::Path, name: &str) -> Value {
    ...
    // 1. 检查必要文�?    let skill_md = skill_path.join("SKILL.md");
        ...
        if let Ok(pkg_content) = std::fs::read_to_string(&package_json) {
            if let Ok(pkg) = serde_json::from_str::<serde_json::Value>(&pkg_content) {
                ...
                if let Some(deps) = pkg.get("dependencies").and_then(|v| v.as_object()) {
                    ...
                    // 检测缺少的依赖（简化版：通过检�?node_modules�?                    let missing_deps = detect_missing_dependencies(&deps_list, skill_path);
                ...
                // 提取 scripts（可能包�?install 后处理等�?                if let Some(scripts) = pkg.get("scripts").and_then(|v| v.as_object()) {
    ...
    // 3. �?SKILL.md frontmatter 提取额外信息
    if has_skill_md {
        if let Some(frontmatter) = parse_skill_frontmatter(&skill_md) {
            // 覆盖或补�?description（SKILL.md �?description 更权威）
    ...
    // 4. 判断 ready 状�?    // Skill ready 需要：1) �?SKILL.md  2) 没有缺少依赖  3) 依赖已安�?    let has_all_deps = result["missingDeps"]
        .as_array()
        .map(|a| a.is_empty())
        .unwrap_or(true);
    ...
    // 5. 检测是否有 node_modules（npm 包已安装�?    let node_modules = skill_path.join("node_modules");
...
fn detect_missing_dependencies(deps: &[String], skill_path: &std::path::Path) -> Vec<String> {
    ...
    if !node_modules.exists() {
        // node_modules 不存在，所有依赖都算缺�?        return deps.to_vec();
    }
    ...
    for dep in deps {
        ...
        if !dep_path.exists() {
            // 可能�?@scope/package 格式，直接检查目�?            missing.push(dep.clone());
        }
    }
    missing
}
...
fn parse_skill_frontmatter(path: &std::path::Path) -> Option<Value> {
    ...
    for line in fm_content.lines() {
        ...
        if let Some(colon_pos) = trimmed.find(':') {
            ...
            // 处理引号包裹的�?            let clean_value = value.trim_matches('"').trim_matches('\'').trim();
