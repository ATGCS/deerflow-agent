...
pub async fn list_agents() -> Result<Value, String> {
    ...
    let main_identity_name = main_profile
        .get("identity")
        .and_then(|i| i.get("name"))
        .and_then(|n| n.as_str())
        .unwrap_or("")
        .to_string();
    let main_identity_emoji = main_profile
        .get("identity")
    ...
    let all_agents = if has_main {
        ...
        if !main_identity_name.is_empty() {
            main_agent["identityName"] = Value::String(main_identity_name.clone());
        }
        if !main_identity_emoji.is_empty() {
            main_agent["identityEmoji"] = Value::String(main_identity_emoji.clone());
        }
        if !main_model.is_null() {
            main_agent["model"] = main_model;
        }
        let mut v = vec![main_agent];
    ...
    let enriched: Vec<Value> = all_agents
        ...
        .map(|mut agent| {
            ...
            if !identity_emoji.is_empty() {
                agent.as_object_mut().map(|o| {
                    o.insert("identityEmoji".to_string(), Value::String(identity_emoji))
                });
            }
            agent
        })
...
pub fn update_agent_identity(
    ...
    let workspace_path = if id == "main" {
        let agents = config
            .as_object_mut()
            .and_then(|o| o.get_mut("agents"))
            .and_then(|a| a.as_object_mut())
            .ok_or("配置格式错误：缺少 agents")?;
        if !agents.contains_key("profiles") || !agents["profiles"].is_object() {
            agents.insert("profiles".to_string(), serde_json::json!({}));
        }
        let profiles = agents
            .get_mut("profiles")
            .and_then(|p| p.as_object_mut())
            .ok_or("profiles 格式错误")?;
        if !profiles.contains_key("main") || !profiles["main"].is_object() {
            profiles.insert("main".to_string(), serde_json::json!({}));
        }
        let main_profile = profiles
            .get_mut("main")
            .and_then(|p| p.as_object_mut())
            .ok_or("main profile 格式错误")?;
        if !main_profile.contains_key("identity") || !main_profile["identity"].is_object() {
            main_profile.insert("identity".to_string(), serde_json::json!({}));
        }
        let identity = main_profile
            .get_mut("identity")
...
pub fn update_agent_model(id: String, model: String) -> Result<String, String> {
    ...
    if id == "main" {
        ...
        let main_profile = profiles
            ...
            .and_then(|p| p.as_object_mut())
            .ok_or("main profile 格式错误")?;
        main_profile.insert("model".to_string(), model_obj);
    } else {
        let agents_list = config
            .get_mut("agents")
            .and_then(|a| a.get_mut("list"))
