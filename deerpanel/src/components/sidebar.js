...
// 异步检测是否有多实例（首次渲染后触发，有多实例时重渲染�?function _checkMultiInstances(el) {
...
const LS_SIDEBAR_COLLAPSED = 'deerpanel_sidebar_collapsed'
...
export function renderSidebar(el) {
  ...
  if (!_delegated) {
    ...
    el.addEventListener('click', (e) => {
      ...
      // 移动端关闭按�?      if (e.target.closest('#btn-sidebar-close')) {
        _closeMobileSidebar()
        return
      }
      // 侧边栏折�?      const collapseBtn = e.target.closest('#btn-sidebar-collapse')
      if (collapseBtn) {
      ...
      // 实例切换�?      const toggleBtn = e.target.closest('#btn-instance-toggle')
      if (toggleBtn) {
    ...
    // 监听实例变化，刷新多实例标记后重新渲�?    onInstanceChange(() => { _checkMultiInstances(el); renderSidebar(el) })
  }
}
