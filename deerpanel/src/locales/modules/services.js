...
export default {
  ...
  configBackupHint: _('备份范围：openclaw.json 主配置文件（含模型、Provider、Gateway 设置）。Agent 数据和记忆文件不在此备份范围内�?, 'Backup scope: deerpanel.json main config (models, providers, Gateway settings). Agent data and memory files are not included.', '備份範圍：openclaw.json 主設定檔案（含模型、Provider、Gateway 設定）。Agent 資料和記憶檔案不在此備份範圍內�?),
  ...
  upgradeConfirm: _('确定要将 DeerPanel 切换到当前面板推荐的稳定{source}{version}吗？\n切换过程�?Gateway 会短暂中断。\n如果你想尝试最新版，请到「关于」页手动切换版本并自测兼容性�?, 'Switch DeerPanel to the panel-recommended stable {source}{version}?\nGateway will be briefly interrupted.\nTo try the latest version, manually switch in the About page.', '確定要將 DeerPanel 切換到目前面板推薦的穩定{source}{version}吗？\n切換過程�?Gateway 會短暂中斷。\n如果你想尝試最新版，請到「關於」頁手動切換版本並自測相容性�?),
