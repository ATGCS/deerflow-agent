...
export default {
  ...
  portTitle: _('服务端口', 'Service Port', '伺服連接�?),
  portLabel: _('端口�?', 'Port', '連接埠號'),
  portHint: _('应用通过这个端口连接 Gateway，默�?18789，一般不需要改', 'Apps connect to Gateway via this port. Default 18789, usually no need to change.', '應用通過这個連接埠連線 Gateway，預�?18789，一般不需要改'),
  accessTitle: _('谁能访问', 'Access Control', '誰能訪問'),
  localOnly: _('仅本机使�?, 'Local Only', '僅本機使�?),
  ...
  lanShareDesc: _('同一网络下的手机、平板等设备也能�?, 'Phones, tablets and other devices on the same network can also use it', '同一網路下的手機、平板等設備也能�?),
  authTitle: _('安全认证', 'Authentication', '安全認證'),
  authMode: _('认证方式', 'Auth Mode', '認證方式', '認証モー�?', '인증 모드'),
  ...
  authPassword: _('密码认证', 'Password', '密碼認證', 'パスワード認�?),
  ...
  tokenPlaceholder: _('不设置则任何人都能调�?, 'Leave empty for no auth', '不設定則任何人都能呼�?),
  ...
  toolsNoneDesc: _('Agent 只能对话，不能调用任何工�?, 'Agent can only chat, cannot use any tools', 'Agent 只能對話，不能呼叫任何工�?),
  sessionsLabel: _('会话可见�?, 'Session Visibility', '對話可见�?),
  sessionsAll: _('所有会话可�?, 'All sessions visible', '所有對話可�?),
  sessionsOwn: _('仅自己的会话', 'Own sessions only', '僅自己的對話'),
  sessionsNone: _('不可�?', 'Not visible'),
  sessionsHint: _('控制 Agent 是否能查看其他会话的上下�?, 'Controls whether Agent can see context from other sessions', '控制 Agent 是否能查看其他對話的上下�?),
  ...
  tailscaleHint: _('如果你用 Tailscale 虚拟局域网，填上地址后远程设备就能通过它访�?Gateway。不用可以留�?, 'If using Tailscale VPN, fill in the address so remote devices can access Gateway through it. Leave empty if not used.', '如果你用 Tailscale 虛擬局域網，填上位址后遠程設備就能通過它訪�?Gateway。不用可以留�?),
  ...
  savedButReloadFailed: _('配置已保存，但重载失�?, 'Config saved, but reload failed', '設定已儲存，但重載失�?),
