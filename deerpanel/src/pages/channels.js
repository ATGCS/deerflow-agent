...
const PLATFORM_REGISTRY = {
  qqbot: {
    ...
    guide: [
      ...
      'DeerPanel 会自动安装 QQBot 社区插件并写入配置，保存后 Gateway 自动重载生效',
    ],
  ...
  dingtalk: {
    ...
    guide: [
      ...
      '回到 DeerPanel 保存。首次保存会自动安装插件，后续保存只更新配置；如果本机已配置 Gateway 鉴权，系统会自动带出对应的 Token 或 Password',
...
async function openBindAgentDialog(pid, page, state) {
  ...
  let agents = []
  try {
    const resp = await api.agentsList()
    agents = Array.isArray(resp?.agents) ? resp.agents : []
  } catch {}
  ...
  const availableAgents = agents.filter(a => !boundIds.has(a.name))
  ...
  const agentOptions = availableAgents.map(a => {
    ...
    return `<option value="${escapeAttr(a.name)}">${a.name}${a.name !== label ? ' — ' + label : ''}</option>`
  }).join('')
...
async function openConfigDialog(pid, page, state) {
  ...
  const agentOptions = agents.map(a => {
    const label = a.description ? a.description.trim() : a.name
    return `<option value="${escapeAttr(a.name)}" ${a.name === currentBinding ? 'selected' : ''}>${a.name}${a.name !== label ? ' — ' + label : ''}</option>`
  }).join('')
