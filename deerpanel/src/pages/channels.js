...
// ── 渠道注册表：面板内置向导，覆�?DeerPanel 官方渠道 + 国内扩展渠道 ──
...
const PLATFORM_REGISTRY = {
  qqbot: {
    ...
    pluginRequired: '@tencent-connect/deerpanel-qqbot@latest',
    pluginId: 'qqbot',
  },
  dingtalk: {
  ...
  feishu: {
    ...
    pluginRequired: '@larksuite/deerpanel-lark@latest',
    pluginId: 'deerpanel-lark',
  ...
  msteams: {
    ...
    configKey: 'msteams',
    pluginRequired: '@deerpanel/msteams@latest',
    pluginId: 'msteams',
  },
  signal: {
    label: 'Signal',
  ...
  matrix: {
    ...
    configKey: 'matrix',
    pluginRequired: '@deerpanel/matrix@latest',
    pluginId: 'matrix',
  },
}
...
function renderConfigured(page, state) {
  ...
  function showPlatformActionMenu(pid, page, state) {
    ...
    // 统计当前 channel+accountId 组合已有�?agent 绑定
  ...
  // 快速为指定 channel+accountId 添加 Agent 绑定（不打开完整配置弹窗�?  async function openAddAgentBindingModalForAccount(pid, accountId, page, state) {
    ...
    modal.querySelector('#btn-quick-bind-save').onclick = async () => {
      ...
      // 检查重�?      const dup = (state.bindings || []).some(b => {
        const bm = b.match || {}
        const bp = bm.peer
  ...
  el.querySelectorAll('.platform-card').forEach(card => {
    ...
    card.querySelector('.platform-card-header')?.addEventListener('click', (e) => {
      // 忽略按钮的点击（按钮有自己的事件�?      if (e.target.closest('button')) return
...
/** deerpanel binding.match.channel �?listConfiguredPlatforms �?id（read_platform_config �?platform�?*/
function bindingChannelToPlatformId(channel) {
  if (!channel) return ''
  if (channel === 'dingtalk-connector') return 'dingtalk'
  if (channel === 'deerpanel-weixin') return 'weixin'
  return channel
}
...
async function openAddAgentBindingModal(agentId, page, state) {
  ...
  const syncAccounts = () => {
    ...
    if (accounts.length) {
      ...
      // 无多账号时，也显示一行提示，方便用户去渠道列表添�?      wrapAcct.style.display = ''
      selAcct.innerHTML = `<option value="">�?${t('channels.noMultiAccount')} �?/option>`
      selAcct.disabled = true
    }
  }

  // 当账号为空时，在 peer hint 里给出提�?  const syncPeerHint = () => {
    const kind = selPeerKind?.value || ''
  ...
  modal.querySelector('#btn-add-bind-save').onclick = async () => {
    ...
    // 检查重复绑�?    const dup = (state.bindings || []).some(b => {
      const bm = b.match || {}
      const bp = bm.peer
      return (b.agentId || 'main') === agentId &&
        bm.channel === channelKey &&
...
/** QQ：展示后端完整诊断（凭证 + Gateway + 插件 + chatCompletions）；可选一键修复插�?*/
function showQqDiagnoseModal(result, options = {}) {
  const accountId = options.accountId != null ? options.accountId : null
  const faqUrl = result?.faqUrl || 'https://q.qq.com/qqbot/deerpanel/faq.html'
...
async function handleGatewayWhatsAppLogin(btn, resultEl, actionDef) {
  ...
  // 检�?Gateway WebSocket 是否已连�?  if (!wsClient.connected || !wsClient.gatewayReady) {
    resultEl.innerHTML = `
  ...
  try {
    ...
    // web login provider is not available = WhatsApp 插件未加�?    const hint = /not available|not supported/i.test(msg)
...
async function openConfigDialog(pid, page, state, accountId) {
      ...
      if (statusEl) {
        api.checkWeixinPluginStatus().then(s => {
          ...
          if (s.installed && s.compatible === false) {
            parts.push(`<span style="color:var(--error);font-weight:600">�?${t('channels.pluginIncompatible') || '插件版本不兼�?}</span>`)
            parts.push(`${t('channels.version')} <strong>${s.installedVersion || '?'}</strong>`)
            parts.push(`<br><span style="color:var(--error);font-size:var(--font-size-xs)">${s.compatError || '请点击「一键安装插件」重新安装兼容版�?}</span>`)
            if (installBtn) {
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          const _flushQr = () => {
            ...
            _qrDone = true
            // 解析 Unicode 半块字符为二值矩�?            const hasHalf = _qrBuf.some(l => /[\u2580\u2584]/.test(l))
            const matrix = []
          ...
          unlistenLog = await listen('channel-action-log', (e) => {
            ...
            if (isQrLine && (actionId === 'login' || actionId === 'install')) {
              ...
              // 检测微信扫�?URL 并渲染为可扫描的二维�?              const weixinUrlMatch = msg.match(/(https:\/\/liteapp\.weixin\.qq\.com\/q\/[^\s]+)/)
          ...
          // runChannelAction 的版本由后端自动检测（微信/QQ 版本号独立于 DeerPanel�?          const output = await api.runChannelAction(pid, actionId, null)
          _flushQr() // 命令结束后刷新残�?QR 缓冲
          if (progressBar) progressBar.style.width = '100%'
          if (progressText) progressText.textContent = '100%'
          toast(t('channels.executionDone'), 'success')
          // 安装完成后刷新插件状�?          if (pid === 'weixin' && actionId === 'install') {
            const statusEl = modal.querySelector('#weixin-plugin-status')
            if (statusEl) {
  ...
  // 加载 Agent 列表（不预选，因为一�?channel+accountId 可以被多�?agent 绑定�?  let agents = []
  try {
  ...
  // 账号标识（多账号）；编辑�?accountId 非空会在 input value 中显�?  const accountIdHtml = supportsMultiAccount ? `
    <div class="form-group">
  ...
  // Agent 绑定选择（一�?channel+accountId 可以绑定到多个不�?agent�?  const agentOptions = agents.map(a => {
    const label = a.identityName ? a.identityName.split(',')[0].trim() : a.id
    // 默认预选第一�?agent，不依赖当前 binding
  ...
  modal.querySelectorAll('[data-channel-action]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        // 微信/QQ 等第三方插件版本号独立，�?pin；run_channel_action �?version 参数仅用�?npx 包名
  ...
  btnVerify.onclick = async () => {
    ...
    // 前端基础检�?    for (const f of reg.fields) {
    ...
    try {
      ...
      if (reg.pluginRequired) {
        ...
        if (!pluginStatus?.installed && !pluginStatus?.builtin) {
          ...
          try {
            // 自动 pin 插件版本：仅 @deerpanel/ 前缀的包�?DeerPanel 版本号同步，其他包（微信 CLI、QQ Bot）版本号独立
            let pluginVersion = null
            if (pluginPackage && pluginPackage.startsWith('@deerpanel/')) {
              try {
                const vInfo = await api.getVersionInfo()
                if (vInfo?.current) pluginVersion = vInfo.current.split('-')[0]
              } catch {}
            }
            // QQ 必须用专用安装命令：官方包目录为 deerpanel-qqbot，与 install_channel_plugin(�? "qqbot") 的备份路径不一�?            if (pid === 'qqbot') {
...
/** 将平�?ID 映射�?deerpanel bindings 中的 channel key */
