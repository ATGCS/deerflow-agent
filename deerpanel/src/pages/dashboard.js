...
export async function render() {
  ...
  // 监听 Gateway 状态变化，自动刷新仪表�?  if (_unsubGw) _unsubGw()
...
async function loadDashboardData(page, fullRefresh = false) {
  ...
  // 分波加载：关键数据先渲染，次要数据后填充，减少白屏等�?  // 轻量调用（读文件）每次都做；重量调用（spawn CLI/网络请求）只在首次或手动刷新时做
  ...
  const coreP = withTimeout(Promise.allSettled([
    ...
    // 版本信息：首次加载或手动刷新时才查询（避�?ARM 设备上频繁查 npm registry�?    (!_dashboardInitialized || fullRefresh || !_dashboardVersionCache) ? api.getVersionInfo() : Promise.resolve(_dashboardVersionCache),
  ]), 15000)
  const secondaryP = withTimeout(Promise.allSettled([
    api.listAgents(),
    api.readMcpConfig(),
    api.listBackups(),
    // getStatusSummary 是最重的调用（spawn deerpanel status --json），只在首次加载时调�?    (!_dashboardInitialized || fullRefresh || !_dashboardStatusSummaryCache) ? api.getStatusSummary() : Promise.resolve(_dashboardStatusSummaryCache),
  ]), 15000).catch(() => [{ status: 'rejected' }, { status: 'rejected' }, { status: 'rejected' }, { status: 'rejected' }])
  const logsP = api.readLogTail('gateway', 20).catch(() => '')

  // 第一波：服务状�?+ 配置 + 版本 �?立即渲染统计卡片
  ...
  // 第三波：日志（最低优先级�?  const logs = await logsP
...
function renderStatCards(page, services, version, agents, config) {
  const cardsEl = page.querySelector('#stat-cards')
  const gw = services.find(s => s.label === 'ai.deerpanel.gateway')
      ...
      <div class="overview-grid">
        ...
        <div class="overview-card">
          ...
          <div class="overview-card-body">
            ...
            <div class="overview-card-meta">${runtimeVer ? 'DeerPanel Runtime' : 'deerpanel.json'}</div>
          </div>
        </div>
      </div>
...
function bindActions(page) {
  ...
  // Control UI 卡片点击 �?打开 DeerPanel 原生面板（用事件委托，因为卡片是动态渲染的�?  page.addEventListener('click', async (e) => {
  ...
  // 概览区域�?Gateway 启动/停止/重启 + ClawApp 导航
  page.addEventListener('click', async (e) => {
    ...
    if (action === 'stop-gw') {
      ...
      try {
        await api.stopService('ai.deerpanel.gateway')
    ...
    if (action === 'restart-gw') {
      ...
      try {
        await api.restartService('ai.deerpanel.gateway')
