...
async function apiCall(cmd, args = {}) {
  if (isTauri) {
    // 桌面端：通过 Tauri IPC 读写 deerpanel.json
    ...
    if (cmd === 'auth_ignore_risk') {
      if (args.enable) {
        ...
        delete cfg.mustChangePassword
        cfg.ignoreRisk = true
        sessionStorage.removeItem('deerpanel_authed')
      } else {
        delete cfg.ignoreRisk
      }
      await api.writePanelConfig(cfg)
...
function checkPasswordStrengthLocal(pw) {
  ...
  const weak = ['123456', '654321', 'password', 'admin', 'qwerty', 'abc123', '111111', '000000', 'letmein', 'welcome', 'deerpanel', 'deerpanel']
...
function renderContent(container, status) {
  ...
  // 当前状�?  const stateIcon = status.hasPassword ? statusIcon('ok', 20) : statusIcon('warn', 20)
...
function bindSecurityEvents(container, status) {
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // 清除默认密码横幅
        sessionStorage.removeItem('deerpanel_must_change_pw')
  ...
  // 无视风险模式开�?  const toggle = container.querySelector('#toggle-ignore-risk')
  const confirmBox = container.querySelector('#ignore-risk-confirm')
  if (toggle && confirmBox) {
    toggle.addEventListener('change', () => {
      if (toggle.checked) {
        // 想开启无视风�?�?显示确认�?        confirmBox.style.display = 'block'
        toggle.checked = false // 先不改，等用户确�?      } else {
        // 想关闭无视风�?�?直接关闭，刷新页面引导设密码
