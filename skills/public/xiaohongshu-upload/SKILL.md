...
1. 先确认 [运行前提](references/runtime-requirements.md) 里的运行前提
2. 再确认 [CLI 契约](references/cli-contract.md) 里的命令契约
3. 执行匹配的 `sau xiaohongshu ...` 命令
4. 如果命令失败，再看 [故障排查](references/troubleshooting.md)
...
- 如果 `sau` 不可用，按 [运行前提](references/runtime-requirements.md) 里的回退方式处理
...
- 运行前提：[references/runtime-requirements.md](references/runtime-requirements.md)
- CLI 契约：[references/cli-contract.md](references/cli-contract.md)
