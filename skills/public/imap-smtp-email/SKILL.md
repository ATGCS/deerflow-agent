...
description: "通过IMAP/SMTP读取和发送邮件。检查新/未读消息、获取内容、搜索邮箱、标记已读/未读、发送带附件的邮件。支持任何IMAP/SMTP服务器，包括Gmail、Outlook、163.com、vip.163.com、126.com、vip.126.com、188.com和vip.188.com。"
official: true
version: 1.0.1
---
...
The `.env` configuration file is automatically managed by zhengfeiClaw Settings (邮箱设置). **Do NOT ask the user to create or edit `.env` — just run the commands directly.** If credentials are wrong, the scripts will return a clear error message; only then should you inform the user to check their email settings.
