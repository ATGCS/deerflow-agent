...
} elseif ($env:zhengfeiClaw_ELECTRON_PATH -and (Test-Path $env:zhengfeiClaw_ELECTRON_PATH)) {
    $NodeBin = $env:zhengfeiClaw_ELECTRON_PATH
    $EnvVars["ELECTRON_RUN_AS_NODE"] = "1"
} else {
    Write-Output '{"success":false,"error":"未找到 Node.js 运行时。请安装 Node.js 或确保 zhengfeiClaw Electron 可用。"}'
    exit 1
}
