---
name: seedance
description: "使用火山引擎Seedance模型生成AI视频。支持文本生成视频(T2V)、图片生成视频(I2V)和音画同步视频生成。当用户想要创建或生成视频时使用此技能。"
official: true
version: 1.0.1
---
...
> ✨ **Node.js 版本**：此脚本使用 Node.js 实现，无需 Python 环境。通过入口脚本自动检测 Node.js 运行时（优先使用系统 node，回退到 zhengfeiClaw 内置运行时），Windows 和 Mac 用户都可以开箱即用。
...
**方式二：通过 zhengfeiClaw 启动时注入**

zhengfeiClaw 会自动读取系统环境变量，确保在启动 zhengfeiClaw 前已设置 `ARK_API_KEY`。
...
zhengfeiClaw 已包含 Node.js 运行时，所有必要的依赖已被自动打包。Windows 和 Mac 用户无需额外配置。
...
**路径说明**：下面的示例使用 `$SKILLS_ROOT` 环境变量来引用脚本路径。zhengfeiClaw 会自动设置这个变量，指向实际的 SKILLs 目录位置，因此无需手动修改路径。
...
或在 zhengfeiClaw 对话中说："用 Seedance 2.0 生成视频..."
