---
name: coding-agent
description: "通过后台进程将编码任务委托给Codex、Claude Code或Pi代理。当需要以下操作时使用：(1) 构建/创建新功能或应用，(2) 审查PR（在临时目录启动），(3) 重构大型代码库，(4) 需要文件探索的迭代编码。不适用于：简单的一行修复（直接编辑）、读取代码（使用read工具）、或在~/clawd工作区工作（切勿在此启动代理）。需要支持pty:true的bash工具。"
metadata:
  {
