...
description: "Web游戏开发测试。当需要构建或迭代Web游戏(HTML/JS)并需要可靠的开发+测试循环时使用：实现小改动、运行基于Playwright的测试脚本、检查截图/文本、查看控制台错误。"
official: true
---
...
```bash
export SKILLS_ROOT="${zhengfeiClaw_SKILLS_ROOT:-${SKILLS_ROOT:-$HOME/Library/Application Support/zhengfeiClaw/SKILLs}}"
...
Installed skills resolve from `$zhengfeiClaw_SKILLS_ROOT` / `$SKILLS_ROOT` (production default: app `userData/SKILLs`, macOS usually `~/Library/Application Support/zhengfeiClaw/SKILLs`).
