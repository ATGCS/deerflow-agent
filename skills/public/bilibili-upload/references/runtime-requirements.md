...
1. 首次运行可能比其他平台更慢，因为要自动准备 biliup
2. 如果网络无法访问 GitHub Release，Bilibili 命令会失败
3. 一旦本地已经准备好 biliup，后续命令会直接复用
4. `sau bilibili login --account <name>` 需要用户自己在本地真实终端里执行
5. 如果终端二维码显示不完整，通常可以直接打开当前目录下的 `qrcode.png` 扫码

## 网络问题处理

如果国内网络访问 GitHub Release 较慢，可先用以下代理辅助访问：
