...
- `DEERFLOW_ALLOW_LOCAL_HOST_READS` - Set to `1`, `true`, or `yes` to allow **read-only** tools (`read_file`, `ls`) and bash path validation to use absolute paths on the host filesystem outside `/mnt/user-data` when using the **local** sandbox (e.g. WSL paths like `/mnt/d/project`, or Windows `D:\...`). Does **not** enable writing outside user-data; intended for self-hosted development only. **Not** a security boundary — do not enable in untrusted or multi-tenant environments.
